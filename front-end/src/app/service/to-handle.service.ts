...
export class ToHandleService {
  ...
  constructor(private http: Http, private apiService: ApiService) {
        this.put_url = apiService.getUrl() + '/handle/update';
        this.unhandle_url = apiService.getUrl() + '/handle/unhandle?eid=';
        this.handled_url = apiService.getUrl() + '/handle/handled?eid=';
        this.delete_url = apiService.getUrl() + '/leave/delete?eid=';
        this.headers = apiService.getHeaders();
    }

  //使用put更新上传请假单，如果请假失败，则获得失败的代码及详细信息
  putHandle(handle: Handle, id: string) {
      const url = `${this.put_url}`;
  ...
  getUnHandlesByid(id: string): Promise<Handle[]> {
    const url = `${this.unhandle_url}`+id;
